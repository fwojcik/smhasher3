...
static inline void crc32c_sw_update( uint32_t * const crcptr, const void * const ptr, size_t len ) {
    ...
    while (len && ((uintptr_t)next & 7) != 0) {
        crc = crc32c_sw_table[0][(crc ^ *next++) & 0xff] ^ (crc >> 8);
        len--;
    }
    while (len >= 16) {
        uint64_t wd1, wd2;
        memcpy(&wd1, next + 0, 8);
        memcpy(&wd2, next + 8, 8);
        wd1 = COND_BSWAP(wd1, isBE());
        // wd2 byteswapping is handled by reordered table indices below
...
extern const uint32_t crc32_long[4][256];
extern const uint32_t crc32_short[4][256];
...
void VCODE_INIT( void ) {
    ...
    if (!vcode_crc_selftest<true>()) {
        printf("VCode CRC32c HW self-test failed!\n");
        exit(1);
    }

    for (int i = 0; i < VCODE_COUNT; i++) {
        resetWithSeed(&vcode_states[i], i);
    }
...
uint32_t VCODE_FINALIZE( void ) {
    ...
    g_resultVCode = getDigest(&vcode_states[2]);
