...
inline uint64_t mix_rev1(uint64_t x) {
	x *= C;
	x ^= x >> 33;
	x *= C;
	x ^= x >> 29;
...
inline uint64_t hash_rev1(const uint8_t* buf, size_t len, uint64_t seed) {
	using namespace internal_rev1;
